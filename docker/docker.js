...
function switchDockerStep(stepNumber, panelType) {
  ...
  const steps = document.querySelectorAll(
    `#docker-step-${panelType}-1, #docker-step-${panelType}-2, #docker-step-${panelType}-3, #docker-step-${panelType}-4`
  );
    ...
    tab.addEventListener('click', function () {
      ...
      if (targetPanel) {
        ...
        if (['docker', 'project', 'code', 'serverless', 'database'].includes(panelType)) {
          // 获取当前显示的步骤
          const activeStepContent = targetPanel.querySelector(
            '.docker-step-content-wrapper[style*="display: block"]'
          );
